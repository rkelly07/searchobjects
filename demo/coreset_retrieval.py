	...
	while True: 
		...
		if loop_finished:
			...
			if v in done_nodes:
				...
				print "leaf node being yielded is", v
				print "importance numbers are", coreset_str.get_importance_keyframes(v)
				yield regions
...
def choose_child(coreset_str, v, done_nodes):
	...
	if len(keyframes) > 0 and len(weights) > 0:
		...
		for c in range(len(children_idx)):
			...
			child_node_importance = coreset_str.get_importance(children_idx[c])
			weights[c] = child_node_importance +1
