...
def retrieve_from_db(filename_part, keyword, threshold, frame_gap, frame_range, fetch_start, is_synthetic_retrieval):
    ...
    if is_synthetic_retrieval:
        ...
        classes = AppClassMapping.objects.filter(class_name__icontains=keyword).filter(class_id__lte=200) #200 classes for RCNN. >200 and < ~7800 for LSDA. 
        #LSDA is not fully implemented now
